...
const {
    ...
    DEFAULT_HELIDON_ENTITY_BP_OPTIONS,
...
function buildEntityGeneratorContext(
    ...
    options = DEFAULT_HELIDON_ENTITY_BP_OPTIONS,
    ...
    return done => {
        new GeneratorTestBuilder('entity')
            ...
            .withOptions({ ...DEFAULT_HELIDON_ENTITY_BP_OPTIONS, ...options })
            .withArguments(['foo'])
            .build(done);
    };
}

function buildServerGeneratorContext(answers = DEFAULT_SERVER_ANSWERS, options = DEFAULT_HELIDON_BP_OPTIONS) {
    return done => {
...
function buildClientGeneratorContext(answers = DEFAULT_CLIENT_ANSWERS, options = DEFAULT_HELIDON_BP_OPTIONS) {
    return done => {
...
function buildCommonGeneratorContext(options = DEFAULT_HELIDON_BP_OPTIONS) {
    return done => {
        new GeneratorTestBuilder('common').withOptions({ ...DEFAULT_HELIDON_ENTITY_BP_OPTIONS, ...options }).build(done);
    };
}
function buildAppGeneratorContext(answers = DEFAULT_SERVER_ANSWERS, options = DEFAULT_HELIDON_BP_OPTIONS) {
    return done => {
